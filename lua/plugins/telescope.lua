  ...
  for _, dir in ipairs(search_dirs) do
    ...
    if handle then
      while true do
        ...
        if type == 'directory' then table.insert(dirs, dir .. '/' .. name) end
      end
    end
  end
      ...
      themes.get_ivy {
        ...
        attach_mappings = function(prompt_bufnr)
          actions.select_default:replace(function()
            ...
            if entry then vim.api.nvim_set_current_dir(entry.value) end
          end)
          return true
        end,
      }
...
-- Grep
map('n', '<leader>fs', function() builtin.live_grep() end, { desc = 'Grep' })

-- Help
map('n', '<leader>fh', function() builtin.help_tags(themes.get_ivy {}) end, { desc = 'Help Pages' })
...
map('n', '<leader>fz', function()
  ...
  colors = vim.tbl_filter(function(c) return not ignore[c] end, colors)
...
map('n', '<leader>gL', function() builtin.git_bcommits() end, { desc = 'Git Log Line' })
...
local function map_lsp(telescope_fn, fallback)
  return function()
    local ok = pcall(function() builtin[telescope_fn]() end)
...
map(
  'n',
  'gri',
  map_lsp('lsp_implementations', vim.lsp.buf.implementation),
...
map(
  'n',
  'gry',
  map_lsp('lsp_type_definitions', vim.lsp.buf.type_definition),
...
map(
  'n',
  'grx',
  map_lsp('lsp_references', vim.lsp.buf.references),
  { desc = 'References', nowait = true }
)

map({ 'n', 'v' }, '<leader>le', function() builtin.diagnostics() end, { desc = 'All Diagnostics' })
